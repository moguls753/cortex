...
import { formatDate, formatTime as formatTimeI18n, bcp47 } from "./i18n/format.js";
import {
  getRecentEntries,
  getDashboardStats,
  getLatestDigest,
...
function categoryAbbr(category: string | null, t: TFunction): string {
  if (!category) return "—";
...
function relativeTime(date: Date, t: TFunction): string {
  ...
  if (minutes < 60) {
    const key = minutes === 1 ? "relative.minutes_ago_one" : "relative.minutes_ago_other";
    return substituteCount(t(key), minutes);
  }
  const hours = Math.floor(minutes / 60);
  if (hours < 24) {
    const key = hours === 1 ? "relative.hours_ago_one" : "relative.hours_ago_other";
    return substituteCount(t(key), hours);
  }
  const days = Math.floor(hours / 24);
  const key = days === 1 ? "relative.days_ago_one" : "relative.days_ago_other";
  return substituteCount(t(key), days);
}

// Catalog entries hold concrete count digits (e.g. "5 minutes ago") so
// tests can compare body against catalog value literally. Production
// substitutes the first digit-sequence with the actual count.
...
function formatTime(date: Date, locale: Locale): string {
  return formatTimeI18n(date, locale, {
    hour: "2-digit",
    minute: "2-digit",
    hour12: false,
  });
...
function renderDigestContent(
  ...
  if (digest) {
    const genTime = formatTime(digest.created_at, locale);
    return `
      <div class="flex items-center gap-1.5 text-[10px] text-muted-foreground mb-3">
        ${iconSparkles("size-3 text-primary")}
        <span>${escapeHtml(t("dashboard.digest_generated_at"))} ${genTime}</span>
      </div>
  ...
  return `
    <div class="flex-1 flex items-center justify-center">
      <p class="text-center italic text-muted-foreground text-sm">
        ${escapeHtml(t("dashboard.no_daily_digest", { time: cronTime }))}
      </p>
    </div>`;
}
...
function renderDigest(
  ...
  const dateLine = formatDate(today, locale, {
    weekday: "long",
    month: "long",
    day: "numeric",
  });
  ...
  return `<div class="h-[55vh] shrink-0 rounded-md border border-border bg-card p-6 flex flex-col">
    <div class="flex items-start justify-between mb-5 shrink-0">
      <div>
        ...
        <h1 class="text-lg font-medium text-foreground mt-0.5 tracking-tight text-balance">${escapeHtml(greeting)} ${escapeHtml(t("dashboard.hero_tagline"))}</h1>
      </div>
    ...
    <div data-digest-panel="daily" class="flex-1 min-h-0 flex flex-col">
      ${renderDigestContent(daily, dailyCronTime, "daily", t, locale)}
    </div>
    <div data-digest-panel="weekly" class="flex-1 min-h-0 flex flex-col hidden">
      ${renderDigestContent(weekly, weeklyCronTime, "weekly", t, locale)}
    </div>
  </div>`;
}

function renderCapture(t: TFunction): string {
  return `
    <div class="flex items-start gap-2">
      <form id="capture-form" class="flex-1">
        <div class="flex items-center gap-3 rounded-md border border-border bg-secondary px-3 py-2 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition-colors">
          ...
          <input type="text" name="text" id="capture-input"
            placeholder="${escapeHtml(t("capture.placeholder"))}"
            autocomplete="off"
      ...
      <a href="/new" class="flex items-center gap-1 rounded-md border border-border px-3 py-2 text-sm text-muted-foreground hover:text-primary hover:border-primary transition-colors shrink-0">
        <span class="text-primary">+</span><span class="hidden sm:inline"> ${escapeHtml(t("new_note.heading"))}</span>
      </a>
    </div>`;
}
...
function renderClientScript(t: TFunction): string {
  // Server-injected translation blob — client-side JS reads these strings
  // instead of importing i18next. Keys here cover SSE renders and capture
  // feedback messages per AC-3.6.
  const i18nBlob = {
    capture: {
      classifying: t("capture.classifying"),
      captured_as: t("capture.captured_as"),
      save_failed: t("capture.save_failed"),
      failed: t("capture.failed"),
    },
    relative: {
      just_now: t("relative.just_now"),
    },
    category_abbr: {
      people: t("category_abbr.people"),
      projects: t("category_abbr.projects"),
      tasks: t("category_abbr.tasks"),
      ideas: t("category_abbr.ideas"),
      reference: t("category_abbr.reference"),
      unclassified: "—",
    },
  };
  // Escape `<` as \u003c so a stray `</script>` in a catalog value can't
  // close the surrounding script tag.
  const blobJson = JSON.stringify(i18nBlob).replace(/</g, "\\u003c");
  return `
<script>
window.__I18N__ = ${blobJson};
(function() {
  ...
  function doCapture() {
    ...
    feedback.innerHTML = '<span class="text-primary animate-pulse">' + I18N.capture.classifying + '</span>';
    fetch('/api/capture', {
      method: 'POST',
    ...
    .then(function(res) {
      ...
      if (res.ok) {
        ...
        if (res.data.classificationError) {
          feedback.innerHTML = '<span class="text-destructive">' + I18N.capture.save_failed + ': ' + esc(res.data.classificationError) + '</span>';
          setTimeout(function() { feedback.innerHTML = ''; }, 8000);
        } else {
          var msg = I18N.capture.captured_as + ' <strong>' + esc(cat) + '</strong>: ' + esc(res.data.name || '') + (conf ? ' (' + conf + ')' : '');
          if (res.data.completed_tasks && res.data.completed_tasks.length > 0) {
            res.data.completed_tasks.forEach(function(t) {
              msg += ' · <strong>' + esc(t.name) + '</strong>';
            });
          }
    ...
    .catch(function() {
      input.disabled = false;
      updateSubmit();
      feedback.innerHTML = '<span class="text-destructive">' + I18N.capture.failed + '</span>';
      setTimeout(function() { feedback.innerHTML = ''; }, 8000);
    });
  }
    ...
    es.addEventListener('entry:updated', function(e) {
      ...
      try {
        ...
        if (badge && d.category) {
          ...
          var abbr = I18N.category_abbr[d.category] || '—';
...
export function createDashboardRoutes(
  ...
  app.get("/", async (c) => {
    ...
    const content = `
      ${renderDigest(dailyDigest ?? null, weeklyDigest ?? null, dailyCronTime, weeklyCronTime, t, locale)}
      ...
      <div class="shrink-0 flex flex-col gap-3 mt-3">
        ...
        <div class="grid grid-cols-12 gap-3">
          ...
          <div class="col-span-8 rounded-md border border-border bg-card px-4 py-3 max-h-[276px] overflow-y-auto scrollbar-thin">
            ...
            ${entries.length > 0 ? `<div class="text-right mt-2"><a href="/browse" class="text-xs text-muted-foreground hover:text-primary transition-colors">${escapeHtml(t("dashboard.view_all"))} &rarr;</a></div>` : ""}
          </div>
        </div>
      </div>

      ${renderClientScript(t)}
    `;

    return c.html(renderLayout("Dashboard", content, "/", healthStatus, c));
  });
