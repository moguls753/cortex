...
| Spec constraints respected | PASS | Model fixed to snowflake-arctic-embed2, 1024 dims, 30s timeout, sequential retry, OLLAMA_URL override |
...
|------------------|----------------|
| AC-1.1–1.4: Ollama /api/embed, 1024-dim | `generateEmbedding()` calls `/api/embed` with model `snowflake-arctic-embed2`, validates 1024 dimensions |
...
| EC-8: Wrong dimensions | `doGenerateEmbedding()` validates `embedding.length !== 1024`, returns null with error log |
