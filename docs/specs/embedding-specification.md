...
- AC-1.1: Given a text input, the system calls Ollama's `/api/embed` endpoint with model `snowflake-arctic-embed2` and returns a 1024-dimensional float array.
...
- AC-2.2: If `snowflake-arctic-embed2` is not present in the model list returned by `/api/tags`, the system pulls it automatically by calling `ollama pull snowflake-arctic-embed2` (via Ollama's `/api/pull` endpoint).
...
- **Technical:** The embedding model is fixed to `snowflake-arctic-embed2`, which produces 1024-dimensional vectors. The database column is `vector(1024)` and cannot accept other dimensionalities.
...
- **Very long text (> 8192 tokens):** snowflake-arctic-embed2 has a context window limit. If the input exceeds the model's maximum, the system should truncate the text to fit within the limit before sending to Ollama. Truncation should happen at a word boundary.
...
- **Text with special characters, emojis, or non-Latin scripts:** snowflake-arctic-embed2 is a multilingual model. The system passes text as-is without stripping special characters. Ollama handles tokenization.
...
- **Embedding model switching at runtime:** The model is fixed to `snowflake-arctic-embed2`. Changing the model would require re-embedding all existing entries. This is listed as a future possibility in the architecture but is not part of this feature.
